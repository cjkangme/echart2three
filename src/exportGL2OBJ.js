function exportGL2OBJ(echarts, chartInstance, componentQuery, opts) {
  // var CREDIT =
  //   "# ECharts v" + echarts.version + "\n" + "# http://echarts.baidu.com/\n";
  ...
  function convertImage(map, textureLib, outName) {
    ...
    var count = (textureLib.__count__[outName] =
  ...
  var componentModel = chartInstance
    .getModel()
  ...
  viewGL.scene.traverse(function (mesh) {
    if (mesh.isRenderable() && mesh.geometry.vertexCount) {
      ...
      materialLib[materialName] = getMaterialParameters(
        mesh.material,
        textureLib
      );
